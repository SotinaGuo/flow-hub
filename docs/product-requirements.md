# 通用申请流程 MVP 产品需求

## 目标

实现一个通用的内部申请流程，使用 Mock 数据模拟人员、部门等基础数据，验证从发起申请到查看结果和统计分析的完整闭环。

## 支持的申请类型

第一版支持四种内置申请类型，并支持基于预设模板的自定义申请类型：

1. 差旅
2. 采购
3. 报销
4. 加班
5. 自定义申请

所有类型共用申请流程，通过类型配置区分表单字段。自定义申请需要填写类型名称，并选择通用类、金额类或时间类模板。

## 功能需求

### 发起申请

- 用户选择申请类型。
- 填写申请人信息和申请内容。
- 差旅包含出发地、目的地、开始日期、结束日期和事由。
- 采购包含采购内容、金额、采购日期和事由。
- 报销包含报销类型、金额、发生日期和事由。
- 加班包含开始日期、结束日期、开始时间、结束时间和事由；开始日期与结束日期相同时表示单日加班。
- 自定义申请包含类型名称，并按所选模板填写日期、金额或时间范围字段和事由。
- 表单需要进行必填和基本格式校验。

### 预览与修改

- 填写完成后可以预览申请信息。
- 预览页需要区分申请人信息和申请内容。
- 用户可以返回对应表单修改后再次预览。
- 确认提交后生成申请记录并进入待审批状态。

### 申请列表

- 展示申请编号、申请类型、申请人、提交时间、金额或时间范围、当前状态。
- 支持按申请类型和状态筛选。
- 支持进入申请详情。
- 需要处理有数据、无数据和筛选无结果状态。

### 申请详情

- 展示完整申请人信息和申请内容。
- 展示申请状态和状态变化信息。
- 待审批申请可以模拟通过、驳回或撤回操作。
- 已完成状态需要展示清晰的结果反馈。

### 状态流转

```text
draft -> pending -> approved
                  -> rejected
pending -> withdrawn
```

非法状态转换不得执行，并需要向用户提供明确反馈。

### 统计报表

- 展示申请总数、待审批数、已通过数和已驳回数。
- 展示不同申请类型的数量分布。
- 使用 Apache ECharts 展示至少一个状态分布图和一个类型分布图。
- 图表需要处理加载、无数据、窗口尺寸变化和销毁场景。

## 非功能需求

- 使用 TypeScript、SvelteKit、Tailwind CSS、Vitest 和 Apache ECharts。
- 禁止使用 React、Vue 和 Angular。
- 人员、部门及申请记录使用本地 Mock 数据。
- 组件需要具备单元测试。
- 页面应具备响应式布局、清晰的信息层级和完整的交互状态。
- 代码应易于将 Mock Repository 替换为真实 API。

## 页面结构

- `/applications`：申请列表
- `/applications/new`：选择类型并创建申请
- `/applications/[id]`：申请详情和状态操作
- `/reports`：统计报表

## 数据模型

核心实体包括：

- `Applicant`：姓名、部门、职位、邮箱
- `ApplicationType`：`travel`、`procurement`、`reimbursement`、`overtime`、`custom`
- `CustomApplicationTemplate`：`general`、`amount`、`time`
- `ApplicationStatus`：`draft`、`pending`、`approved`、`rejected`、`withdrawn`
- `Application`：编号、类型、申请人、内容、状态、提交时间、更新时间、状态历史
- `ApplicationFormData`：按申请类型承载不同表单字段

申请类型字段使用判别联合或类型守卫建模，避免在组件中使用无约束的字段访问。

## 实现边界

数据访问通过 `application-repository` 统一封装，页面和组件不直接依赖 Mock 数组。纯业务计算放在 `lib/application` 下，Mock 数据集中放在 `lib/mocks` 下，组件只负责交互和展示。

第一版使用客户端本地 Mock 数据，Repository 接口保持异步形式，便于后续替换为 API 请求。`draft` 仅表示填写过程中的客户端预览状态，不提供独立的草稿保存和恢复功能。自定义申请仅支持预设模板，不包含自定义字段设计器。第一版不包含真实登录、权限系统、后端持久化和多人审批协作。

## 测试策略

- 使用 Vitest 测试表单校验、五类申请类型字段解析、状态转换和统计聚合。
- 为申请类型选择、表单错误提示、预览返回编辑和列表筛选补充 Svelte 组件测试。
- 对 ECharts 封装验证无数据处理、初始化、窗口尺寸变化和销毁行为。
- 完成后执行测试、类型检查、Lint 检查和生产构建。

## UI/UX 要求

- 采用适合内部运营工具的紧凑、清晰、可扫描布局。
- 使用语义化 HTML、可访问的表单控件和键盘可操作的交互。
- 所有主要操作提供 hover、focus、loading、disabled 和错误反馈。
- 页面需要兼容移动端，不允许文本溢出、内容遮挡或布局跳动。
- 使用 Tailwind CSS 建立一致的间距、颜色和状态样式。
